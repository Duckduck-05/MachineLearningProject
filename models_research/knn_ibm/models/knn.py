...
class KNearestNeighbors:
    def __init__(self, x: np.ndarray, k: int, y: np.ndarray, similarity: str = "euclid", device="cpu"):
        if device == "cuda":
            ...
            self.CUDA = False
        self.device = device
        self.x = x  # num_samples x num_features (e.g., seq_length from dataset)
        self.y = y  # num_samples x 1 (binary labels: 0 or 1)
        self.k = k
    ...
    def _euclid(self, x1, x2):
        '''Return the Euclidean distance between `x1` and `x2`. `x1.shape` should be `(batch_size, 1, num_features)` while `x2.shape` should be `(num_samples, num_features)`'''
        if self.CUDA:
    ...
    def predict(self, sample, reduction="argmax", *, train=False):
        ...
        if train:
            indices = indices[1:]  # Exclude self if training
        ...
        # Vote for the class (binary: 0 or 1)
        vote = np.zeros([batch_size, 2])  # Adjusted to 2 classes (0 or 1)
        for i in range(2):
...
class WeightedKNearestNeighbors:
    def __init__(self, x: np.ndarray, k: int, y: np.ndarray, similarity: str = "euclid", weights=1, learning_rate=0.01, device="cpu", train_split_ratio=0.7):
        self.x = torch.tensor(x, device=device) if not isinstance(x, torch.Tensor) else x.to(device)
        self.k = k
        self.y = torch.tensor(y, device=device) if not isinstance(y, torch.Tensor) else y.to(device)
    ...
    def distance(self, x: torch.Tensor, x2=None, numpy=True) -> np.ndarray | torch.Tensor:
        ...
        if x2 is None:
            x2 = self.x
        x2 = x2 if isinstance(x2, torch.Tensor) else torch.tensor(x2, device=self.device)
        if len(x.shape) == 1:

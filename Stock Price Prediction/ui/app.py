...
import pandas as pd
import requests # Để gọi API FastAPI
import plotly.graph_objects as go
from datetime import datetime, timedelta # Thêm timedelta
import os
import time # For potential sleep after trigger
...
TICKERS_DISPLAY_CONFIG = {
    "^GSPC": {"display_name": "S&P 500 Index"}, # Sử dụng key mà API và config backend dùng
    "IBM": {"display_name": "IBM Stock"}
}
AVAILABLE_TICKER_KEYS = list(TICKERS_DISPLAY_CONFIG.keys())
REGRESSION_MODELS_TO_DISPLAY = ["xgboost", "random_forest", "lstm", "knn"]
CLASSIFICATION_MODELS_TO_DISPLAY = ["knn", "random_forest"] # Add RFC to classification models

st.set_page_config(layout="wide", page_title="Stock & Index Price Prediction")
st.title("📈 Stock & Index Price Prediction Dashboard")
st.markdown("---")

# --- Sidebar for Ticker Selection ---
st.sidebar.header("🎯 Select Ticker")
# Sử dụng key từ TICKERS_DISPLAY_CONFIG vì API endpoint cũng dùng key này
...
if REGRESSION_MODELS_TO_DISPLAY:
    ...
    for i, model_name in enumerate(REGRESSION_MODELS_TO_DISPLAY):
        with reg_cols[i]:
            ...
            # Gọi hàm fetch_latest_prediction cho mỗi model khi trang tải hoặc ticker thay đổi
            ...
            else: # Trường hợp fetch_latest_prediction trả về None bất ngờ (dù không nên)
        ...
        with class_cols[i]:
            ...
            if classification_data and "error" not in classification_data:
                ...
                if classification_data.get("predicted_direction"):
                    # Use predicted_direction field directly
...
def load_history_data_from_api(ticker_key_for_api: str, days_limit: int = 365):
    ...
    try:
        ...
        # API trả về prediction_date là string, Pydantic model HistoryDataPointAPI có predicted_price, actual_price
        df['prediction_date'] = pd.to_datetime(df['prediction_date'])
        df.set_index('prediction_date', inplace=True)
        df.sort_index(ascending=True, inplace=True)
        # Đảm bảo các cột giá là numeric
        if 'predicted_price' in df.columns:
            df['predicted_price'] = pd.to_numeric(df['predicted_price'], errors='coerce')
        if 'actual_price' in df.columns: # Tên cột này phải khớp với Pydantic model HistoryDataPointAPI
...
days_of_history_slider = st.slider(
    "Days of History to Display:", min_value=30, max_value=730, # Max 2 years
...
if not history_df_all_models.empty:
    ...
    if not history_df_regression.empty:
        ...
        # Filter data for chart based on selected models for predictions
        # Actual price should always be shown if available
        chart_data_to_plot = history_df_regression.copy() # Start with all regression history

        fig = go.Figure()

        # Plot Actual Prices from the 'actual_price' column
        actual_prices_series = chart_data_to_plot['actual_price'].dropna().sort_index()
        # To avoid duplicate actual price lines if multiple models have predictions for same date
        # we can group by date and take the first non-null actual price
        ...
        color_map = {"xgboost": "orange", "random_forest": "lightgreen", "lstm": "violet", "knn": "magenta"}
        dash_map = {"xgboost": "solid", "random_forest": "dashdot", "lstm": "dot", "knn": "longdash"}
        ...
        # Display Table Data, ensure using correct column names from API
        st.subheader("Historical Data Table")
        table_df_to_display = history_df_regression[
            history_df_regression['model_used'].isin(selected_models_for_chart)
        ][['model_used', 'predicted_price', 'actual_price']].copy() # Ensure these columns exist
        
        table_df_to_display.rename(columns={
            'model_used': 'Model Used',
            'predicted_price': 'Predicted Price', # This name is used by API
            'actual_price': 'Actual Price'      # This name is used by API
        }, inplace=True)
...
if st.sidebar.button("Trigger Full Data Update Pipeline", key="trigger_pipeline_button"):
    with st.spinner("Requesting data pipeline trigger... This may take several minutes. Check server logs for progress."):
        try:
            ...
            response = requests.post(trigger_url, timeout=20) # POST request, longer timeout
            response.raise_for_status()
            ...
            # No st.experimental_rerun() immediately, as background task takes time.
        except requests.exceptions.HTTPError as http_err:

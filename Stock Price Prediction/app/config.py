...
RFC_MODEL_NAME_TEMPLATE = "{ticker_key}_rfc_{problem_type}.pkl"
# Problem types are "classification" or "regression"

def get_model_path(ticker_key: str, model_type: str, problem_type: str) -> str:
    """Get path to model file based on ticker, model type, and problem type."""
    if ticker_key == "^GSPC":  # Replace special characters for filenames
        ticker_key = "GSPC"
        
    if not os.path.exists(MODELS_STORE_DIR):
        os.makedirs(MODELS_STORE_DIR)
...
KNN_PARAMS = {
    ...
    "split_ratio": [0.60, 0.20, 0.20],  # Train/val/test split ratios
    "batch_size": 64,  # Batch size for training
    "learning_rate": 0.05,  # Learning rate for weighted KNN
...
RFC_PARAMS = {
    ...
    "max_features": 4,
    "random_state": 36,
    "prediction_window": 30  # Days to predict ahead (one of 10, 15, 30, 60, 90)
}

...
def train_rfc_model(ticker_key: str, window: int = None):
    ...
    try:
        ...
        logger.error(f"Error training RFC model: {str(e)}")
        return None

def train_all_rfc_models(windows=None):
    """Train RFC models for all tickers with specified windows"""
    if windows is None:
